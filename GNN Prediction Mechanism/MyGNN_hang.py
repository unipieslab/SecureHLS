...
with open("..\\kfold_hang\\out.txt","w") as f: print("",file=f) #  create a statistics file to dump information

with open("..\\kfold_hang\\train_pred.txt","w") as f: print("",file=f)
with open("..\\kfold_hang\\val_pred.txt","w") as f: print("",file=f)
with open("..\\kfold_hang\\val_log_pred.txt","w") as f: print("",file=f)
with open("..\\kfold_hang\\test_pred.txt","w") as f: print("",file=f)
...
dataset = torch.load("dataset_h_04_05.pt",map_location='cuda')
...
for fold in range(5):
    ...
    class EarlyStopper:
        ...
        def early_stop(self, validation_loss):
            if validation_loss < self.min_validation_loss:
                ...
                self.counter = 0
                torch.save(model.state_dict(),"..\\kfold_hang\\model_kfold_hang_"+str(fold)+".pth")
                ...
                if self.counter >= self.patience:
                    return True
            return False 
    with open("..\\kfold_hang\\out.txt","a") as f: 
        print("--- Fold %d ---",fold,file=f)
    with open("..\\kfold_hang\\train_pred.txt","a") as f: 
        print("--- Fold %d ---",fold,file=f)
    with open("..\\kfold_hang\\val_pred.txt","a") as f: 
        print("--- Fold %d ---",fold,file=f)
    with open("..\\kfold_hang\\val_log_pred.txt","a") as f: 
        print("--- Fold %d ---",fold,file=f)
    ...
    def train():
       ...
       for data in train_loader[fold]:  # Iterate in batches over the training dataset.
          ...
          with open("..\\kfold_hang\\train_pred.txt","a") as f: print("   Prediction: "+str(float(torch.exp(out.data)))+" Ground Truth: "+str(float(torch.exp(data.y)))+"\n",file=f)
    ...
    def val(loader):     
        with torch.no_grad():
            ...
            for data in loader:  # Iterate in batches over the training/test dataset.
                ...
                with open("..\\kfold_hang\\val_log_pred.txt","a") as f: print(str(data.d_index)+"   Prediction: "+str(float(out.data))+" Ground Truth: "+str(float(data.y))+"\n",file=f)
                with open("..\\kfold_hang\\val_pred.txt","a") as f: print(str(data.d_index)+"   Prediction: "+str(float(torch.exp(out.data)))+" Ground Truth: "+str(float(torch.exp(data.y)))+"\n",file=f)
    ...
    def test(loader):     
          with torch.no_grad():
              ...
              for data in loader:  # Iterate in batches over the training/test dataset.
                  ...
                  with open("..\\kfold_hang\\test_pred.txt","a") as f: print(str(data.d_index)+"   Prediction: "+str(float(torch.exp(out.data)))+" Ground Truth: "+str(float(torch.exp(data.y)))+"\n",file=f)
    ...
    for epoch in range(1, 1001):  
        ...
        with open("..\\kfold_hang\\train_pred.txt","a") as f: print(f'\nEpoch:{epoch:02d}\n',file=f)
        with open("..\\kfold_hang\\val_pred.txt","a") as f: print(f'\nEpoch:{epoch:02d}\n',file=f)
        with open("..\\kfold_hang\\val_log_pred.txt","a") as f: print(f'\nEpoch:{epoch:02d}\n',file=f)
    ...
    model_best.load_state_dict(torch.load("..\\kfold_hang\\model_kfold_hang_"+str(fold)+".pth",map_location='cuda'))
    
    
    test_loss,test_mae = test(test_loader)
    with open("..\\kfold_hang\\test_pred.txt","a") as f: 
        print("--- Test MSE for fold "+str(fold)+" : "+str(test_mae)+"  ---",file=f)
    
    #torch.save(model,"model_kfold_hang_"+str(fold)+".pth")
    del model_best
