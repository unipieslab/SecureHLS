...
set out [open "$d/M3_out.txt" "w"] 

# Set headers for M3_out.txt
...
#after 9999999
while  {![file exists "$d/M3_registers.txt"]} {

	after 30000
	puts "Waiting for M3_registers.txt...\n\n"
}

# Get victim signals from newregisters.txt
set regfile [open "$d/M3_registers.txt" "r"] 
set victim_regs_1 {}
set victim_times_1 {}
set victim_regs_2 {}
set victim_times_2 {}
...
foreach i $victim_regs_1 ii $victim_times_1 j $victim_regs_2 jj $victim_times_2 k $victim_regs_3 kk $victim_times_3 {

	puts [format {%s	%s	%s	%s	%s	%s} $i $ii $j $jj $k $kk]

	set out [open "$d/M3_out.txt" "a"]
	...
	# Retrieve effect on output and write on M3_out.txt
