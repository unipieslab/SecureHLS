...
add_force {/top_function/input_r} -radix bin {*** 0ns}
...
set out [open "$d/M2_out.txt" "w"] 

# Set headers for M2_out.txt
...
#after 9999999
while  {![file exists "$d/M2_registers.txt"]} {

	after 30000
	puts "Waiting for M2_registers.txt...\n\n"
	
}

# Get victim signals from newregisters.txt
set regfile [open "$d/M2_registers.txt" "r"] 
set victim_regs_1 {}
set victim_times_1 {}
set victim_regs_2 {}
set victim_times_2 {}
...
foreach i $victim_regs_1 ii $victim_times_1 j $victim_regs_2 jj $victim_times_2 {

	puts [format {%s	%s	%s	%s} $i $ii $j $jj]

	set out [open "$d/M2_out.txt" "a"]
	...
	add_force {/top_function/ap_rst} -radix hex {0 0ns} 
	add_force {/top_function/ap_start} -radix hex {1 0ns}		
	#add_force {/top_function/input_r} -radix bin {11010001 0ns} 
	...
	if {$jj > $ii} {
		...
		set remaining_time [expr {$fault_time_2 - $fault_time_1}]
		...
		# Retrieve effect on output and write on M2_out.txt
